...
function isolated(){assert.notEqual(readlinkSync('/proc/self/ns/net'),readlinkSync('/proc/1/ns/net'),'Fresh disconnected network namespace required');}
async function main(){isolated();const mode=process.argv[2];
 if(mode==='--probe'){assert.equal(process.argv.length,3);const input=JSON.parse(readFileSync(0,'utf8'));assert.equal(process.getuid(),input.uid);assert.equal(process.getgid(),input.gid);assert.ok(process.getgroups().every(gid=>gid===input.gid));for(const item of input.matrix)await checkBind(item);return;}
 ...
 for(const owner of before.apps){const entry=passwd.find(row=>Number(row[2])===owner.uid);assert.ok(entry);const gid=Number(entry[3]);
  const result=spawnSync('/usr/bin/setpriv',[`--reuid=${owner.uid}`,`--regid=${gid}`,'--clear-groups',process.execPath,new URL(import.meta.url).pathname,'--probe'],
   {input:JSON.stringify({uid:owner.uid,gid,matrix:bindMatrix(before,owner.uid,phase)}),encoding:'utf8',timeout:15000,env:{PATH:'/usr/bin:/bin'}});
